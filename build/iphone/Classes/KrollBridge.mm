...
@implementation MyTwitterClientObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mytwitterclient);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any MyTwitterClient patches in JS before a context is loaded... nice for 
...
{
	// create MyTwitterClient global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_mytwitterclient = [[MyTwitterClientObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mytwitterclient];
	
	NSString *_mytwitterclientNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_mytwitterclientNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mytwitterclient valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.mytwitterclient.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
